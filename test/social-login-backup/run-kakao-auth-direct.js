...
/**
 * 카카오 인증 테스트 직접 실행 스크립트
 * 
 * Jest 없이 직접 테스트 함수를 호출하여 실행합니다.
 */
    ...
    try {
        ...
        try {
            ...
            if (KAKAO_ID && KAKAO_PASSWORD) {
                ...
                // 이미 리다이렉션된 경우 체크 (이미 로그인되어 있는 경우 바로 리다이렉션 될 수 있음)
                if (initialUrl.includes(process.env.KAKAO_REDIRECT_URI.split('?')[0]) && initialUrl.includes('code=')) {
                    console.log('이미 인증이 완료되어 리다이렉션되었습니다!');
                    ...
                    try {
                        ...
                        if (authCode) {
                            console.log('즉시 추출된 인증 코드:', authCode.substring(0, 10) + '...');
                            console.log('인증 코드 길이:', authCode.length);
                        } else {
                            const codeMatches = initialUrl.match(/[\?&]code=([^&]+)/);
                            if (codeMatches && codeMatches[1]) {
                                authCode = codeMatches[1];
                                console.log('정규식으로 즉시 추출된 인증 코드:', authCode.substring(0, 10) + '...');
                            } else {
                                throw new Error('인증 코드를 찾을 수 없습니다.');
                            }
                        }
                        
                        // 이미 인증 코드를 얻었으므로 로그인 과정 건너뛰기
                        console.log('로그인 과정 건너뛰기 (이미 인증됨)');
                    } catch (error) {
                    ...
                    try {
                        ...
                        if (!loginIdElement) {
                            ...
                            throw new Error('로그인 아이디 입력 필드를 찾을 수 없습니다.');
                        }
                        
                        // 아이디 입력
                        await loginIdElement.sendKeys(KAKAO_ID);
                        ...
                        // 비밀번호 입력
                        await passwordElement.sendKeys(KAKAO_PASSWORD);
                        ...
                        console.log('예상 리다이렉션 URI:', process.env.KAKAO_REDIRECT_URI);
                        ...
                        await driver.wait(async () => {
                            ...
                            const hasRedirectUri = currentUrl.includes(redirectUriBase);
                        ...
                        console.error('로그인 과정 오류:', loginError.message);
                        ...
                        try {
                            ...
                            console.error('URL 확인 실패:', e.message);
                        }
                    ...
                    try {
                        ...
                        if (authCode) {
                            console.log('추출된 인증 코드:', authCode.substring(0, 10) + '...');
                            console.log('인증 코드 길이:', authCode.length);
                        } else {
                            console.error('URL에서 인증 코드를 찾을 수 없습니다:', currentUrl);
                            
                            // URL에 code가 있는지 확인하는 다른 방법 시도
                            const codeMatches = currentUrl.match(/[\?&]code=([^&]+)/);
                            if (codeMatches && codeMatches[1]) {
                                authCode = codeMatches[1];
                                console.log('정규식으로 추출된 인증 코드:', authCode.substring(0, 10) + '...');
                            }
                        }
                    } catch (error) {
                        console.error('URL 파싱 오류:', error.message);
                        // URL 파싱 오류시 다른 방법으로 코드 추출 시도
                        const codeMatches = currentUrl.match(/[\?&]code=([^&]+)/);
                        if (codeMatches && codeMatches[1]) {
                            authCode = codeMatches[1];
                            console.log('대체 방법으로 추출된 인증 코드:', authCode.substring(0, 10) + '...');
                        }
                    }
