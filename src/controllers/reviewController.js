...
router.get('/:shopId/reviews', async (req, res) => {
  try {
    ...
    console.log(`GET /:shopId/reviews - 파라미터:`, { shopId, page, limit });
    ...
    try {
      ...
      console.log(`GET /:shopId/reviews - 성공:`, { count: reviews.length });
...
/**
 * 리뷰 상세 조회 API
 * GET /:shopId/reviews/:reviewId
 */
router.get('/:shopId/reviews/:reviewId', async (req, res) => {
  try {
...
router.post('/:shopId/reviews', verifyToken, async (req, res) => {
  try {
    ...
    // 리뷰가 null이면 마지막 리뷰라도 조회해서 반환
    let reviewData = review;
    if (!reviewData) {
      const reviews = await ReviewService.getReviewsByShopId(shopId, 1, 1);
      reviewData = reviews && reviews.length > 0 ? reviews[0] : null;
    }
...
/**
 * 리뷰 수정 API
 * PUT /:shopId/reviews/:reviewId
 * 인증 필요
 */
router.put('/:shopId/reviews/:reviewId', verifyToken, async (req, res) => {
  try {
    const shopId = parseInt(req.params.shopId);
    const reviewId = parseInt(req.params.reviewId);
 ...
 * 인증 필요
 */
router.delete('/:shopId/reviews/:reviewId', verifyToken, async (req, res) => {
  try {
    const shopId = parseInt(req.params.shopId);
    const reviewId = parseInt(req.params.reviewId);
...
/**
 * 리뷰 이미지 업로드 API
 * POST /:shopId/reviews/:reviewId/images?index=1
 * 인증 필요
 */
router.post('/:shopId/reviews/:reviewId/images', verifyToken, upload.single('image'), async (req, res) => {
  try {
    const shopId = parseInt(req.params.shopId);
    const reviewId = parseInt(req.params.reviewId);
...
/**
 * 리뷰 이미지 삭제 API
 * DELETE /:shopId/reviews/:reviewId/images/:imageIndex
 * 인증 필요
 */
router.delete('/:shopId/reviews/:reviewId/images/:imageIndex', verifyToken, async (req, res) => {
  try {
    const shopId = parseInt(req.params.shopId);
    const reviewId = parseInt(req.params.reviewId);
...
/**
 * 리뷰 좋아요 추가 API
 * POST /:shopId/reviews/:reviewId/like
 * 인증 필요
 */
router.post('/:shopId/reviews/:reviewId/like', verifyToken, async (req, res) => {
  try {
    const shopId = parseInt(req.params.shopId);
    const reviewId = parseInt(req.params.reviewId);
...
// shopId 없이 reviewId만 사용하는 이미지 업로드
...
// shopId 없이 reviewId만 사용하는 이미지 삭제
router.delete('/:reviewId/images/:imageIndex', verifyToken, async (req, res) => {
  try {
    ...
    try {
      await ReviewImageService.deleteImage(reviewId, userId, imageIndex);
      return res.status(200).json({
        success: true,
        message: '이미지가 성공적으로 삭제되었습니다.'
      });
...
// /api/v1/reviews/:reviewId 단일 리뷰 조회 라우트 추가
router.get('/:reviewId', async (req, res) => {
  try {
    ...
    console.error('리뷰 단일 조회 중 오류 발생:', error);
    return res.status(500).json({
      success: false,
      errorCode: ERROR_CODES.INTERNAL_ERROR.code,
      message: ERROR_CODES.INTERNAL_ERROR.message
...
// /api/v1/shops/:shopId/reviews alias 라우트 추가 (목록 조회)
...
// /api/v1/shops/:shopId/reviews alias 라우트 추가 (리뷰 생성)
router.post('/shops/:shopId/reviews', verifyToken, async (req, res) => {
  try {
    const shopId = parseInt(req.params.shopId);
    if (!req.user || !req.user.userId) {
      return res.status(400).json({
